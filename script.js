...
// JM: Consider your naming convention here. Took me a minute to find the function that handles comparison of your sequence with player's sequence.
function handleQuadrant(evt) {
  ...
  if (quadrantArr[clickCounter] === evt.target) {
    ...
    //if quadrant clicked does not match
...
// JM: From a UX/UI perspective, I found it hard to tell what was illuminated or when the sequence was complete. Consider reducing the timeouts and use a different affect for mouseover from the illumination.
...
function quadrantRandomizer() {
  if (quadrantArr.length < round || quadrantArr.length === 0) {
    ...
    // JM: Consider how you could make this more DRY. For each condition you're using the same setup. Really you only need to change the quadrant you're affecting and what gets pushed to your quadrantArr.
    // JM: Rather than pushing the entire quadrant to the array, consider using id to track them to reduce the space complexity.
    if (randomized < 25) {
      //highlight
